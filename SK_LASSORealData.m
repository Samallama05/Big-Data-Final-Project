     ...
     if Reach_reg(i).exclude == 0
        switch trigger
            case 1
                ...
                model_info = model_info(:,[2,3,4,7,8,9, 12, 13, 14]);
...
%%
%Predictors = normalize([pos_x pos_y pos_z vel vel_x vel_x_up vel_x_down...
...
%    Reach_reg(1).synth_tot]);

%Predictors = normalize([pos_x pos_y pos_z vel vel_x vel_x_up vel_x_down...
%   vel_y vel_y_up vel_y_down vel_z vel_z_up vel_z_down acc...
%   acc_x acc_x_up acc_x_down acc_y acc_y_up acc_y_down acc_z acc_z_up acc_z_down...
%   Reach_reg(1).synth_tot]',"range")';
...
torque_names = ["m_shoulder_el", "m_shoulder_an", "m_elbow_an", ...
    "m_shoulder_el_speed", "m_shoulder_an_speed", "m_elbow_speed",...
"m_shoulder_el_torque", "m_shoulder_ang_torque", "m_elbow_torque"]; 
...
Predictor_names = ["pos_x", "pos_y", "pos_z", "vel", "vel_x", "vel_x_up",  ...
    ...
    "acc_y_up", "acc_y_down", "acc_z", "acc_z_up", "acc_z_down", torque_names];
...
legend("Empirical FR","","","Synthesized FR","","")
xlabel("Time (samples)")
ylabel("Firing Rate (FR)")
title("Mean FR alligned to reach in empirical and synthesized data")
...
xticks(1)
xticklabels(["R^2 of Synthetic Data"])
% Give this a y axis label and title:
ylabel("R^2 of fit")
title("Fit to emperical data with synthesized kinematics")
...
%%
%savedir = 'single_var_fits';
%mkdir(savedir)
% 
%for i = 1:size(Predictors,2)
%    tester = Predictors(:,i);
%    mdl = fitlm(tester,Responses);
%    h= figure(); hold on;
%    plot(mdl)
%    title(Predictor_names(i) + " vs. FR, R^2: " + mdl.Rsquared.Adjusted)
%xlabel(Predictor_names(i))
%ylabel('FR')
%saveas(h, savedir+"/" + Predictor_names(i) + ".png")
%close all
%end
%
%%%
